      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {profiles.map((profile, i) => (
            ...
            >
              <div className="rounded-xl bg-[var(--field)] p-5 h-full">
                <div className="flex items-start gap-3">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--green)_10%,transparent)] mt-0.5">
                    <Check size={14} className="text-[var(--green)]" />
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-[var(--fg)]">{profile.title}</h3>
                    <p className="mt-1 text-sm text-[var(--fg2)]">{profile.desc}</p>
                  </div>
                </div>
              </div>
            </motion.div>
