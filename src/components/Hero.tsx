...
export default function Hero() {
  ...
  return (
    <section className="relative pt-6 pb-10 sm:pt-16 lg:pt-20 lg:pb-12">
      <div className="relative before:absolute before:bottom-0 before:h-px before:w-[200vw] before:-left-[100vw] before:bg-[var(--sep)]" />

      <div className="px-4 sm:px-6 lg:px-8">
        {/* Section label */}
        <div className="mb-7">
          <span className="mac-eyebrow">App Mastery</span>
        </div>

        {/* Main card */}
        <div className="relative isolate overflow-hidden rounded-[20px] border-[0.5px] border-[var(--sep)] bg-[var(--card)]">
          <div
            className="pointer-events-none absolute inset-0"
            style={{
              background:
                "radial-gradient(80% 60% at 80% 0%, var(--accent-glow), transparent 60%)",
            }}
          />

          <div className="relative grid lg:grid-cols-[3fr_2fr] gap-0">
            {/* Left: all text content */}
            <div className="relative px-5 py-8 sm:px-12 sm:py-20 lg:py-24">
              <motion.h1
                className="text-4xl/tight sm:text-5xl/tight lg:text-[3.5rem]/tight font-bold tracking-[-0.04em] text-balance text-[var(--fg)]"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
              >
                De Zéro à une <span className="mac-accent">App Rentable</span> sur
                l&apos;App Store en 28 Jours
              </motion.h1>

              <motion.p
                className="mt-6 text-lg/7 text-[var(--fg2)] max-w-lg"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.15 }}
              >
                La méthode exacte qui m&apos;a permis de passer de 0 à 400 000€/an de
                revenus récurrents avec une seule app. Sans écrire une ligne de code,
                grâce au vibe coding IA.
              </motion.p>

              <motion.ul
                className="mt-6 space-y-2 text-lg text-[var(--fg2)]"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <span className="text-base font-medium text-[var(--fg2)]">
                  Ils ont déjà lancé leur app
                </span>
              </motion.div>

              <motion.p
                className="mt-6 text-[var(--accent2)] italic text-lg font-medium"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.6, delay: 0.65 }}
              >
                Et le plus fou ? Ce n&apos;est même pas la partie la plus
                impressionnante de la méthode...
              </motion.p>
            </div>

            {/* Right: phone mockup */}
            <motion.div
              className="relative hidden lg:flex items-center justify-center p-8 border-l-[0.5px] border-[var(--sep)] overflow-hidden"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {/* Glow */}
              <div
                className="pointer-events-none absolute bottom-1/4 left-1/2 -translate-x-1/2 w-72 h-72 rounded-full blur-3xl"
                style={{ background: "var(--accent-glow)" }}
              />

              {/* Phone frame */}
              <div className="relative w-72">
                <div className="relative rounded-[2.5rem] bg-gray-900 p-2 shadow-2xl outline outline-white/10">
                  {/* Notch */}
                  ...
                  <div className="rounded-[2rem] bg-white overflow-hidden">
                    ...
                    <div className="bg-gradient-to-b from-gray-950 to-gray-900 px-4 py-5 space-y-4">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#3898ff] to-[#0a84ff] flex items-center justify-center">
                      ...
                      <div className="rounded-xl bg-white/5 p-4">
                        ...
                        <div className="flex items-end gap-0.5 h-16 mt-3">
                          {[30, 45, 35, 55, 65, 50, 75, 85, 70, 90, 95, 100].map((h, i) => (
                            <div
                              key={i}
                              className="flex-1 rounded-sm bg-gradient-to-t from-[#0a84ff]/60 to-emerald-400/60"
                              style={{ height: `${h}%` }}
                            />
                          ))}
                        </div>
                      ...
                      <div className="space-y-2">
                        {["Nouvel abonné Premium", "Paiement reçu +9.99€", "5 étoiles reçues", "Abonné upgradé Pro", "Nouveau téléchargement", "Paiement reçu +9.99€"].map((item, i) => (
                          <div key={i} className="flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2">
                            <div className={`w-1.5 h-1.5 rounded-full ${i === 0 ? "bg-[#0a84ff]" : i === 1 ? "bg-emerald-400" : i === 2 ? "bg-amber-400" : i === 3 ? "bg-violet-400" : i === 4 ? "bg-pink-400" : "bg-emerald-400"}`} />
                    ...
                    <div className="bg-gray-950 border-t border-white/5 px-6 py-2 flex justify-between">
                      {["Accueil", "Stats", "Profil"].map((tab, i) => (
                        <div key={i} className="flex flex-col items-center gap-0.5">
                          <div className={`w-4 h-4 rounded-full ${i === 0 ? "bg-[#0a84ff]/30" : "bg-white/10"}`} />
                          <span className={`text-[7px] ${i === 0 ? "text-[#3898ff]" : "text-gray-600"}`}>{tab}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex justify-center gap-3 mt-4">
                  <div className="flex items-center gap-1.5 rounded-lg bg-[var(--field)] px-3 py-1.5 border-[0.5px] border-[var(--sep)]">
                    <svg className="w-4 h-4 text-[var(--fg)]" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                    </svg>
                    <span className="text-[10px] text-[var(--fg2)] font-medium">App Store</span>
                  </div>
                  <div className="flex items-center gap-1.5 rounded-lg bg-[var(--field)] px-3 py-1.5 border-[0.5px] border-[var(--sep)]">
                    <svg className="w-4 h-4 text-[var(--fg)]" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <span className="text-[10px] text-[var(--fg2)] font-medium">Google Play</span>
                  </div>
                </div>
              </div>
            </motion.div>
