          ...
          <div className="relative rounded-xl bg-white/5 overflow-hidden">
            ...
            <div className="relative hidden md:block">
              <table className="w-full table-fixed">
                <thead>
                  <tr>
                    <th className="text-left px-5 py-4 text-lg font-semibold text-gray-300">
                      Critère
                    </th>
                    <th className="text-center px-5 py-4 text-lg font-semibold text-gray-300">
                      Tutos YouTube / Udemy
                    </th>
                    <th className="text-center px-5 py-4 text-lg font-semibold text-gray-300">
                      Bootcamp / École
                    </th>
                    <th className="text-center px-5 py-4 text-lg font-semibold text-sky-400 relative">
                ...
                <tbody>
                  {criteria.map((row, i) => (
                    ...
                    >
                      <td className="px-5 py-4 text-lg font-medium text-white">
                        {row.label}
                      </td>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-lg text-gray-300">YouTube / Udemy</span>
                      <div className="flex items-center gap-2">
                        {row.youtube && (
                          <span className="text-lg text-gray-300">{row.youtube}</span>
                        )}
                        <StatusIcon type={row.youtubeIcon} />
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-lg text-gray-300">Bootcamp / École</span>
                      <div className="flex items-center gap-2">
                        {row.bootcamp && (
                          <span className="text-lg text-gray-300">{row.bootcamp}</span>
                        )}
                        <StatusIcon type={row.bootcampIcon} />
                      </div>
                    </div>
                    <div className="flex items-center justify-between rounded-md bg-sky-500/10 px-2 py-1">
                      <span className="text-lg font-semibold text-sky-400">App Mastery</span>
                      <div className="flex items-center gap-2">
                        {row.appMastery && (
                          <span className="text-lg font-semibold text-white">{row.appMastery}</span>
                        )}
                        <StatusIcon type={row.appMasteryIcon} />
                      </div>
                    </div>
